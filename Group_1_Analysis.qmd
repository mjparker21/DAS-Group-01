...
@fig-GLMfull shows each of the parameters that are included in the final model. The selected model with the lowest AIC includes variabkes `Veg`, `Sex`, `Employment`, `AgeGroup` and an interaction term between `Sex` and `Agegroup`. The equation for the full fitted model is formally written below:
...
\end{equation*}
```
Where, $\mathbb{I}$ is an indicator function and $p = \text{Prob}(\text{Obese})$. @eq-fittedmodel highlights how the log-odds of an obese individual change by the various lifestyle and socio-economic factors. This gives us estimates for the log-odds, and therefore odds, for each individual in the survey.

In order to examine ranges of plausible log-odds of obesity for each of the predictors in @eq-fittedmodel, we can observe the 95% confidence interval bounds given in @fig-GLMfull . Note that these confidence intervals also highlight significance of the parameters, which gives the same result as the conclusions reached from the p-values in @fig-GLMfull.
...
Finally, to see if our model predicts as we would expect it to, we produce plots of the probability that an individual self-evaluates as obese by the various lifestyle and socio-economic factors. These plots are displayed in @fig-probs1 . From the plots it is clear that the probabilities of an individual being obese vary by these factors and, in fact, these probabilities match our analysis in @sec-exploranalysis. For example, we observe that the probability of an individual being obese is higher for females than males and the probability of obesity is lowest in the youngest age group, followed by the oldest.
...
We also observed an interaction between an indivual's sex and age. There appeared to be an increase in the odds of an individual evaluating as obese in every category compared to females in the youngest age group. The highest average increase in the odds was observed in males aged 55-64. So we conclude that females aged 16-24 have the lowest odds of being obese.
